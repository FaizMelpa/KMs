...
export interface Product {
  ...
  images: string[]; // array of base64, max 3
  notes: string | null;
  createdAt: string;
}
...
function loadCategories(): Category[] {
  try { return JSON.parse(localStorage.getItem(KEYS.categories) || "[]"); } catch { return []; }
}
...
export const db = {
  ...
  deleteCategory(id: number): boolean {
    ...
    saveProducts(loadProducts().map((p) => p.categoryId === id ? { ...p, categoryId: null, categoryName: null } : p));
    return true;
  },
  getProducts(search?: string, categoryId?: string): Product[] {
    ...
    if (search) {
      ...
      prods = prods.filter((p) => p.name.toLowerCase().includes(q) || (p.notes || "").toLowerCase().includes(q));
    }
    if (categoryId === "uncategorized") prods = prods.filter((p) => p.categoryId === null);
    else if (categoryId && categoryId !== "all") prods = prods.filter((p) => p.categoryId === parseInt(categoryId));
    return prods.sort((a, b) => a.name.localeCompare(b.name));
  },
  addProduct(data: Omit<Product, "id" | "categoryName" | "createdAt">): Product {
    const prods = loadProducts();
    const prod: Product = { ...data, id: nextId(KEYS.seq_prod), categoryName: null, createdAt: new Date().toISOString() };
    prods.push(prod);
    saveProducts(prods);
    return prod;
  },
  ...
  exportBackup(): string {
    return JSON.stringify({ version: 2, categories: loadCategories(), products: loadProducts(), exportedAt: new Date().toISOString() }, null, 2);
  },
